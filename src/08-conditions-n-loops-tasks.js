...
function getDigitalRoot(num) {
  let firstRound = num.toString().split('').map((x) => Number(x)).reduce((total, item) => total + item);
  if (firstRound > 9) {
    firstRound = firstRound.toString().split('').map((x) => Number(x)).reduce((total, item) => total + item);
  }
  return firstRound;
}
